...
public class AvatarService extends ServiceImpl<AvatarMapper, Avatar> {
    ...
    public Page<Avatar> selectPage(int index, int pageSize) {
        Page<Avatar> page = lambdaQuery().page(new Page<>(index, pageSize));
        
        // 确保URL是绝对路径
        for(Avatar avatar : page.getRecords()) {
            if (avatar.getUrl() != null && !avatar.getUrl().startsWith("http")) {
                avatar.setUrl(avatar.getUrl());
            }
        }
